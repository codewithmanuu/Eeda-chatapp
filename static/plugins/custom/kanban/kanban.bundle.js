...
// ====================================================================================================================================================================
                                    ...
                                    // Create a container for title and img elements
                                    ...
                                    container.innerHTML = '<div class="kanban-title-board">' + p.title + '</div>';
                                    var image_div_container = document.createElement('div')
                                    var image_div = document.createElement('div')
                                    var image_div_1 = document.createElement('div')
                                    var image_div_2 = document.createElement('div')

                                    image_div_1.classList.add('edit_button')
                                    image_div_1.setAttribute('title', 'Edit Tasklist');
                                    image_div_2.setAttribute('title', 'Delete TaskList');
                                    image_div.style.display = "flex"
                                    image_div.style.gap = "5px"

                                    image_div_2.classList.add('edit_button')
                                    
                                    if(user === "employee"){
                                        image_div_container.style.display = "none"
                                    }else{
                                        image_div_container.style.display = "block"
                                    }
                                    
                                    // Create first img element
                                    var img1 = document.createElement("img");
                                    img1.src = '/static/media/svg/icons/General/Edit.svg'; 
                                    img1.alt = 'Description for the first image'; 
                                    
                                    // onclick funtion image tag1
                                    
                                    img1.addEventListener('click', function() {
                                        editTasklist(this);
                                    });
                                    function editTasklist(element) {
                                        $('#modal-update-task').modal('show');
                                        var dataId = $(element).closest('.kanban-board').data('id');
                                        var projId = $('#data-id').data('projId');
                                        var taskListId = dataId;
                                        $('#modalTaskListId').val(taskListId);
                                 
                                
                                        $.ajax({
                                            url: projId + '/edittasklist', 
                                            type: 'GET',
                                            dataType: 'json',
                                            data: {
                                                'projId': projId,
                                    ...
                                    var boardId = currentUrl.match(/\/boards\/(\d+)/);
                                    var extractedId = parseInt(boardId[1], 10);
                                    var projId = extractedId
                                    console.log(projId)
                                 
                                    var aTag_image2 = document.createElement('a')
                                    aTag_image2.setAttribute('href','javascript:void(0)')
                                    aTag_image2.setAttribute('id', projId +'/deletettasklist')
                                    aTag_image2.setAttribute('class', 'aff')

                                    


                                    var img2 = document.createElement("img");
                                    img2.src = '/static/media/svg/icons/General/Trash.svg';  // Replace 'your_second_image_link.jpg' with the actual link
                                    img2.alt = 'Description for the second image';  // Provide a description for accessibility

                                    img2.addEventListener('click', function() {
                                        deleteTasklist(this);
                                    });
                                    function deleteTasklist(element){
                                        var dataId = $(element).closest('.kanban-board').data('id');
                                        var projId = $('#data-id').data('projId');
                                        var task_list_id = dataId;
                                        swal.fire({
                                            text: "Are You Sure ?",
                                            icon: "error",
                                            ...
                                            $.ajax({
                                                ...
                                                success: (data) => {
                                                    ...
                                                    setInterval(function() {
                                                        window.location.replace('/boards/'+data.projectId);
                                                    }, 1000);
                                    ...
                                    container.appendChild(image_div_container);
                                    image_div_container.appendChild(image_div)
                                    image_div.appendChild(image_div_1);
                                    image_div.appendChild(image_div_2);
                                    image_div_1.appendChild(img1);
                                    image_div_2.appendChild(aTag_image2);
                                    aTag_image2.appendChild(img2)

                                    // Append the container to the header
                                    v.appendChild(container);

                                    // Append the header to the document body or another parent element
...
// =================================================================================================================================================
                                    ...
                                    // console.log(b,"====bbb=bbbbb=b=b=b=b=b=b=b=b=b=b=b",g)
                                    ...
                                    p.item)) 
                                    console.log(e,"=================",p)
                                    for (var y = 0; y < p.item.length; y++) {
                                        var w = p.item[y];
                                        // console.log(w,"=============ppppppppppppppppppp")
                                        if (w) { // Check if w is defined
                                            var E = document.createElement("div");
                                            E.classList.add("kanban-item");
                                            w.id && (E.dataset.eid = w.id);
                                            
                                            if (w.class && Array.isArray(w.class)) {
                                    ...
                                    if (true) {
                                        ...
                                        var O = document.createTextNode(c || "Add Task");
                                        C.setAttribute("data-id", w ? w.tl_id : null); // Check if w is defined
                                        C.setAttribute("class", l || "kanban-title-button btn btn-default btn-xs btn-open-task-add-modal");
                                        C.appendChild(O);
                                        u ? T.appendChild(C) : T.appendChild(C);
                                        a(C, p.id);
                                    }
                                    if(user === "employee"){
